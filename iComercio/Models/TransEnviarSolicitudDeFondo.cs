...
{
    class TransEnviarSolicitudDeFondo:Operacion
    {

        public TransEnviarSolicitudDeFondo()             
        {
        }
        

        public TransEnviarSolicitudDeFondo(int OperacionID, string Nombre, string Descripcion):
        ...
        public async override Task<bool> Enviar(BusinessLayer bl,Transmision tran)
        {
           ...
           if (sf != null)
           {
               var sfr = await bl.TransmitirSolicitudDeFondo(sf);
               if (sfr != null)
               { 
                    sf.SolicitudFondoIDRemoto = sfr.SolicitudFondoIDRemoto;
